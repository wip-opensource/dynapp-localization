...
function getLocalizationPrefixes() {
  ...
  const rootJson = JSON.parse(require('fs').readFileSync(path.join(getWorkspacePath(), dataItemPath, vscode.workspace.getConfiguration('dynapp-localization').get("rootfilename") || 'root.json'), 'utf8'));
  if (!rootJson) {
    return null
  }
