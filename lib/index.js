...
function StartPlaceInMessage(value, range) {
  PlaceInMessages(value, range)
}

function PlaceInMessages(value, range) {
...
function getUnderlineRanges(document) {
  ...
  if (json.bindings || json.itembindings) {
    ...
    bindings.forEach((binding) => {
      if (binding.view.includes(":background"), binding.view.includes(":color"), binding.view.includes(":hintcolor"), binding.view.includes(":bordercolor"), binding.view.includes(":actioncolor")){
        return
      }
      if (!binding.source) {
        return
...
module.exports = {
  StartPlaceInMessage, getMatchingStrings, getValueFromMessages, getLocalizationPrefixes, getMessagesPaths, getUnderlineRanges
}
